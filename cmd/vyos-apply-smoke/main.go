...
func runWithContext(ctx context.Context, args []string, out io.Writer, now func() time.Time) int {
	...
	logf(out, "warning: minimal-managed uses normal policy and may delete interfaces bridge, nat source, service dhcp-server, service dns forwarding, and service ssh; networking or SSH access may briefly flap")
...
func logCleanupGuidance(out io.Writer) {
	...
	logf(out, "minimal-managed uses normal apply policy and may delete interfaces bridge, nat source, service dhcp-server, service dns forwarding, and service ssh")
